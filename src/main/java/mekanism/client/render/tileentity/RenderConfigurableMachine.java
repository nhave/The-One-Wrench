/*** 
 * Original code made by aidancbrady as part of the mod Mekanism
 * I do not take credit for anything done with this class file.
 * This is me using a hijacked version utilizing the Forge mod load ordering.
 * Crash Reports containing this class must be reported to this mod not Mekanism.
 **/
...
public class RenderConfigurableMachine<S extends TileEntity & ISideConfiguration> extends TileEntitySpecialRenderer<S>
{
	...
	public void render(S configurable, double x, double y, double z, float partialTick, int destroyStage, float alpha)
	{
		...
		if(pos != null && !itemStack.isEmpty() && itemStack.getItem() instanceof ItemOmniwrench && ((ItemOmniwrench)itemStack.getItem()).getWrenchMode(itemStack) == ModIntegration.modeMekanism && MekanismHandler.getState(itemStack).isConfigurating())
		{
			type = MekanismHandler.getState(itemStack).getTransmission();
		}
		else if(pos != null && !itemStack.isEmpty() && itemStack.getItem() instanceof ItemConfigurator && ((ItemConfigurator)itemStack.getItem()).getState(itemStack).isConfigurating())
		{
			type = ((ItemConfigurator)itemStack.getItem()).getState(itemStack).getTransmission();
		}
		
		if(type != null && pos != null && !itemStack.isEmpty())
		{
